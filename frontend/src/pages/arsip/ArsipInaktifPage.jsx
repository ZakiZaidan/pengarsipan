...
import { Search, FolderOpen, Eye, Trash2, Download, FileText, Database } from 'lucide-react';
import { formatTanggal, confirmAlert } from '../../utils/helpers';
import toast from 'react-hot-toast';

export default function ArsipInaktifPage() {
  ...
  const fetchArsips = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/arsip?status_retensi=inaktif&cari=${search}&kode_klasifikasi=${classification}`);
      setArsips(res.data.data || res.data || []);
    } catch (err) {
      toast.error('Gagal mengambil daftar arsip inaktif');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMusnahkan = async (id, e) => {
    e.stopPropagation();
    const isConfirmed = await confirmAlert(
      'Musnahkan Arsip?',
      'WARNING: Anda yakin ingin memusnahkan arsip ini? Status retensi arsip akan diubah menjadi MUSNAH secara permanen untuk keperluan audit.',
      'Musnahkan',
      true
    );
    ...
    try {
      ...
      toast.success('Arsip berhasil dimusnahkan secara administratif');
      fetchArsips();
    } catch (err) {
      toast.error('Gagal memusnahkan arsip');
    }
  };
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {arsips.length === 0 ? (
            <div style={{ padding: '48px', textAlign: 'center', color: 'var(--slate-400)' }}>
              <div style={{ marginBottom: '12px' }}><FolderOpen size={40} style={{ opacity: 0.5 }} /></div>
              Tidak ada arsip inaktif terdaftar.
            </div>
          ) : (
                ...
                <tbody>
                  {arsips.map((arsip) => (
                    <tr key={arsip.id} onClick={() => handleOpenDetail(arsip.id)} style={{ cursor: 'pointer' }}>
                      ...
                      <td>{formatTanggal(arsip.tanggal_inaktif)}</td>
                        ...
                        <div className="actions" style={{ justifyContent: 'flex-end' }}>
                          ...
                          {user?.peran === 'ketufor' && (
                            <button 
                              className="btn btn-ghost btn-sm" 
                              title="Musnahkan"
                              onClick={(e) => handleMusnahkan(arsip.id, e)}
                            >
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', zIndex: 1000, padding: '20px', justifyContent: 'center' }}>
          <div className="card" style={{ width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div className="card-header">
              <h3 className="card-title">Metadata Arsip Resmi (Inaktif)</h3>
            ...
            <div className="card-body">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', background: 'var(--slate-50)', padding: '16px', borderRadius: 'var(--radius)', border: '1px solid var(--slate-200)', marginBottom: '24px', fontSize: '13px' }}>
                ...
                <div>
                  <div style={{ color: 'var(--slate-500)', marginBottom: '2px' }}>Dipindah Inaktif Pada:</div>
                  <div style={{ color: 'var(--warning-600)', fontWeight: '600' }}>{formatTanggal(selectedArsip.tanggal_inaktif)}</div>
                </div>
                <div>
